...
: "${CUDA_VISIBLE_DEVICES:?Set CUDA_VISIBLE_DEVICES to four free physical GPUs}"
if [[ "$(awk -F, '{print NF}' <<<"$CUDA_VISIBLE_DEVICES")" -ne 4 ]]; then
  echo "ERROR: CUDA_VISIBLE_DEVICES must contain exactly four comma-separated GPUs." >&2
  exit 2
fi
...
JOB_NAME="${JOB_NAME:-df93_t210_${MODE}_$(date +%Y%m%d_%H%M%S)}"
...
ARTIFACT_DIR="${ARTIFACT_DIR:-$OMNI_CACHE_DIR/nsys/omnidreams_df93_4gpu/fa3_ab/$JOB_NAME}"
