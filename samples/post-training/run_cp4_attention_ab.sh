...
# Four-GPU, CP=4/FSDP=4 causal-attention A/B launcher.
#
...
JOB_NAME="${JOB_NAME:-df93_t210_cp4_${MODE}_$(date +%Y%m%d_%H%M%S)}"
...
ARTIFACT_DIR="${ARTIFACT_DIR:-$OMNI_CACHE_DIR/nsys/omnidreams_df93_4gpu/cp4_attention/$JOB_NAME}"
...
PY
  ...
  sha256sum \
    ...
    "$SCRIPT_DIR/run_cp4_attention_ab.sh" \
