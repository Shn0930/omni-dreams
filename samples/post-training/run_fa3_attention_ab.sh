...
# Configurable single-node attention-backend/checkpoint A/B launcher.
#
# All three modes use contiguous CP. NPROC is inferred from
# CUDA_VISIBLE_DEVICES unless set explicitly; CP_SIZE and FSDP_SIZE default
# to NPROC and can be chosen independently when they divide NPROC.
#
#   CUDA_VISIBLE_DEVICES=0,1,2,3 NPROC=4 CP_SIZE=1 FSDP_SIZE=4 \
#     bash samples/post-training/run_fa3_attention_ab.sh fa3-sac
#   CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 NPROC=8 CP_SIZE=2 FSDP_SIZE=8 \
...
case "$MODE" in
  ...
  fa3-sac)
    ...
    ;;
  *)
    echo "Usage: $0 {flex|fa3|fa3-sac} [Hydra overrides ...]" >&2
    exit 2
    ;;
esac
shift
...
ARTIFACT_DIR="${ARTIFACT_DIR:-$OMNI_CACHE_DIR/nsys/omnidreams_df93_${NPROC}gpu/fa3_ab/$JOB_NAME}"
...
"$PYTHON" -m torch.distributed.run \
  ...
  model.config.net.training_context_parallel_strategy=contiguous \
